...
export class GatewayService {
  static async saveSetupDraft(payload: any) {
    ...
    return { message: 'Draft saved securely' };
  }

  static async deployTournament(payload: any) {
    ...
    if (batches.length > 0) {
      await Promise.all(batches.map((b, i) => b.commit().catch((err: any) => {
        throw err;
      })));
    }
    ...
    return { message: 'Tournament deployed successfully' };
  }

  static async assignStaff(payload: any) {
    ...
    if (!acquired) {
      throw new Error('CONFLICT: This assignment is currently being processed by another user.');
    }
  ...
  static async updateSchedule(payload: any) {
