  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      if (regex1.test(content) || regex2.test(content)) {
        content = content.replace(regex1, `from '@taikaix/backend/$1'`);
        content = content.replace(regex2, `import('@taikaix/backend/$1')`);
        modified = true;
      }

      if (modified) {
        fs.writeFileSync(fullPath, content, 'utf8');
        console.log('Updated to @taikaix/backend in', fullPath);
      }
    }
  }
}
...
console.log('Done mapping to @taikaix/backend');
